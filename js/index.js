...
Snake.prototype.strategies = {
    move: function (format) {  //这个参数用于决定是否删除蛇尾，当传了这个参数就表示要做的事情是吃
        //创建新身体（在旧蛇头的位置）
        ...
        newHead.viewContent.style.transform = 'rotate('+ this.direction.rotate +'deg)';  //更改蛇头方向
        newHead.create();

        //蛇身上的每一个方块的坐标也要更新(newPos最前面塞newHead的位置)
...
function createFood() {
    ...
    food.pos = [x,y];       //存储生成食物坐标，用于跟蛇头要走的下一个点作对比

    //设计模式中的单例模式：只生成一个，改变位置即可
...
Game.prototype.init = function () {
    ...
    document.onkeydown = function (ev) {
        if (ev.which == 37 && snake.direction != snake.directionNum.right) {  //按左键则不能往右走
